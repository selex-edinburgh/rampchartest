...
#First No of Waypoint pair = x coordinate.
#Second No of Waypoint pair = y coordinate
#routeType = "ClockwiseSquare"   
    waypoints = [(0),(0),       #waypoints[0] Start Waypoint Heading 0 deg North
                 (0),(1000),    #waypoints[1]
                 (1000),(1000), #waypoints[2]
                 (1000),(0),    #waypoints[3]
                 (0),(0),       #waypoints[4]
                 (0),(1000),    #wawpoints[5] Required to action final turn
         	]
    endWpNo = 4                 #last waypoint number
...
def handle_rollovers(angDataLt,angDataRt,prevAngDataLt,prevAngDataRt,\
    ...
    if (angDataLt <341) and (prevAngDataLt >683):  #positive rollover sectors
        odomDistLt = odomDistLt + 1024 + changeLt
    elif (angDataLt >683) and (prevAngDataLt <341):#negative rollover sectors
        odomDistLt = odomDistLt - 1024 + changeLt
    else:
...
'''
    ...
    It then starts a continuous loop, within which commands are actioned endlessly
    until the program is stopped.
    The motor code section listens for particular keyboard key presses. 
        's' will start the forward calibration movement
        'r' will start reverse movement.
        'space' will stop the movement.
    The odometer code section
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:    #detect a key press
                ...
                if event.key == pygame.K_r:     #key r 'START reverse'
                    fwd  = 97                   #reverse
                    turn = 127                  #no turn movement
                if event.key == pygame.K_c:     #key c 'Change Direction'
                    if mode == "Clockwise":
                        mode = "AntiClockwise"  #Anticlockwise Square
                    elif mode == "AntiClockwise":
                        mode = "Clockwise"      #Clockwise Square
