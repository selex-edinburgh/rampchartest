...
try:
    import RPi.GPIO as GPIO             #Set up GPIO Interface
except RuntimeError:
...
#reset()                     #Initialise all variables to start condition
...
def reset():
    ...
    print "Reset Function Activated - Not yet complete"
    #WPNo = 0
 ...
 #       print "                            Fwd/Lt",(FwdBk-128), (LtRt-128)
    ...
    if elapsed_time > 0.18:                      #Test message if > 190 ms
        if (MainLoopCounter+4)%8 == 0:     #Modulus = 0 print every ten loops +4		
            print "ms per loop= ", int(elapsed_time*1000)  #Print if >?ms 
