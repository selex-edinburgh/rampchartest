#Update system time using LXTerminal  sudo date 0413144915  #MMDDHHMMYY
...
#HALT Logic     -----------------------------------------
    ...
    if MainLoopCounter == 1:            #Programe Running and First Pass
        ...
        logfile=open('/home/pi/datalogtst.csv','w')     #Open datalog file and delete old values

        #Write time & date file header
        logfile.write(time.strftime("%H:%M:%S %b %d %Y\n")) #Write time & date file header+new line
        #Write column headings \n
        logfile.write("Time, WPNo, LegMode, Dist2Go, Hdg2Go, DistLt, DistRt, Hdg, FwdBk, LtRt, LoopTime,\n")
...
#Waypoint Navigation Logic    --------------------------------
    ...
    if WPNo > EndWpNo:				    #Finish
        ...
        ser.write(chr(FwdBk))	                    #Output to Motor Drive Board	    			
        ser.write(chr(LtRt))                        #Output to Motor Drive Board
        print "WPNo  LegMode  Dist2Go  Hdg2Go  DistLt  DistRt  Hdg  FwdBk  LtRt LoopTime"
        print( WPNo,' ',LegMode,' ',int(Dist2Go),"",int(Hdg2Go),int(DistLt),int(DistRt),int(Hdg),FwdBk,LtRt,int(ElapsedTime*1000))
...
#DATALOGGING        ----------------------------------------- 
    ...
    logfile.write(", %d" % LegMode)                     #Write data to file
    ...
    logfile.write(", %.2f" % ElapsedTime)                 #Write floating point data to file
    logfile.write(",\n")                                #New line
    logfile.close()                                     #Close file
