...
'''
Pygame is a set of Python modules designed for writing games. We are using it for display and graphics as it is very powerful, fast and easy to use. Its libaries contain core functions written in C and Assembly code which run very fast (can easily be 100 or more times faster than python code).
Pygame is free. Released under the General Public License, which means you can create open source, free, freeware, shareware, and commercial games with it. See the licence for full details.
'''

pygame.init() 			# Set display parameters of the Pygame display window
...
'''
	This function will set up the serial interface used to communicate 
	with the Rampaging Chariot Master Motor Drive Board.
	It will hopefully explain any errors that occur by using an (except) function.
	If you are using a Raspberry pi 3B the serial port is called: port = '/dev/ttyS0'
	If you are using a Raspberry Pi 2B  the serial port is called: port = '/dev/ttyAMA0' 
...
'''
	This function listens for a reply from the Master Motor Drive Board
...
def main():
    ...
    while True:                 #The main loop/ infinite loop/ game loop
